...
class TestUM:
	...
	def test_actorOn(self):
		assert requests.post(self.url+'actuator/action/1/on', data={}, cookies={},headers=self.headers).text == '{"result":"done"}'

	def test_actorOff(self):
		assert requests.post(self.url+'actuator/action/1/off', data={}, cookies={},headers=self.headers).text == '{"result":"done"}'
	...
	def test_actorNotexitAction(self):
		assert requests.post(self.url+'actuator/action/Lamp1NoGood/on', data={}, cookies={},headers=self.headers).text == '{"result":"not defined"}'
